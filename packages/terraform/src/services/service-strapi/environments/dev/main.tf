...
module "modules" {
  ...
  # cloud storage
  cloud_storage_bucket_name = lookup(var.storage_strapi, "name")

  # cloud sql
  cloud_sql_database_version       = lookup(var.db_strapi, "version")
  cloud_sql_database_name          = lookup(var.db_strapi, "name")
  cloud_sql_database_user_name     = lookup(var.db_strapi, "user")
